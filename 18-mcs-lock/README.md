...
Оказывается, что замечательное свойство алгоритма MCS блокировки, которое заключается в том, что он ждет на своем узле, 
...
Рассуждение выше касается только ожидания блокировки, которая в данный момент занята другим потоком. 
...
* [`CorrectnessTest`](test/CorrectnessTest.kt) проверяет правильность работы блокировки в соответствии с FCFS и
  корректность использования методов park/unpark.
